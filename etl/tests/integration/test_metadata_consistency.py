...
class TestMetadataConsistency:
    ...
    def test_metadata_consistent_spark_trino(self, spark_session, trino_connection, clean_nessie):
        ...
        # Trino metadata
        trino_count = get_table_row_count(trino_connection, "consistency_test", "trades")
        trino_schema = get_table_schema(trino_connection, "consistency_test", "trades")
        trino_columns = [col["name"] for col in trino_schema]

        # Assert consistency
        assert spark_count == trino_count, f"Row count mismatch: Spark={spark_count}, Trino={trino_count}"
        assert spark_columns == trino_columns, (
            f"Schema mismatch:\n  Spark columns: {spark_columns}\n  Trino columns: {trino_columns}"
        )

    def test_metadata_consistent_after_write(self, spark_session, trino_connection, clean_nessie):
        ...
        write_data(
            ...
            "write_consistency_test",
            "trades",
            generate_trades(20, seed=6002),
            schema,
        )

        # Verify initial consistency
        spark_count_initial = read_table(spark_session, "write_consistency_test", "trades").count()
        trino_count_initial = get_table_row_count(trino_connection, "write_consistency_test", "trades")
        ...
        spark_session.catalog.refreshTable("lakehouse.write_consistency_test.trades")
        spark_count_after = read_table(spark_session, "write_consistency_test", "trades").count()
        assert spark_count_after == 21, f"Spark expected 21 rows after Trino write, got {spark_count_after}"
        ...
        write_data(
            ...
            "write_consistency_test",
            "trades",
            generate_trades(5, seed=6003),
            schema,
        )
        trino_count_after = get_table_row_count(trino_connection, "write_consistency_test", "trades")
        assert trino_count_after == 26, f"Trino expected 26 rows after Spark write, got {trino_count_after}"

    def test_schema_visible_all_engines(self, spark_session, trino_connection, clean_nessie):
        ...
        trino_schema_before = get_table_schema(trino_connection, "schema_vis_test", "trades")
        ...
        spark_session.sql("ALTER TABLE lakehouse.schema_vis_test.trades ADD COLUMNS (clearing_house STRING)")
        spark_session.sql("ALTER TABLE lakehouse.schema_vis_test.trades ADD COLUMNS (fee DECIMAL(18,4))")
        ...
        trino_schema_after = get_table_schema(trino_connection, "schema_vis_test", "trades")
        trino_cols_after = [col["name"] for col in trino_schema_after]

        assert "clearing_house" in trino_cols_after, f"Trino missing 'clearing_house': {trino_cols_after}"
        assert "fee" in trino_cols_after, f"Trino missing 'fee': {trino_cols_after}"

        # Full schema order should match
        assert spark_cols_after == trino_cols_after, (
            f"Schema mismatch after evolution:\n  Spark: {spark_cols_after}\n  Trino: {trino_cols_after}"
        )
