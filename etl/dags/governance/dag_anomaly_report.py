...
def _export_compliance_pdf(**context) -> str:
    ...
    try:
        ...
        from_ts = int((datetime.now(timezone.utc) - timedelta(days=1)).timestamp() * 1000)
        to_ts = int(datetime.now(timezone.utc).timestamp() * 1000)

        url = (
            f"{_GRAFANA_REPORTER_URL}/render"
            f"?dashboard=bcbs239-compliance&from={from_ts}&to={to_ts}"
        )
